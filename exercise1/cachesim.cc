...
{
  size_t idx = (addr >> idx_shift) & (sets - 1);
  std::queue<size_t>& q = fifo_queues[idx];

  if (q.size() < ways) {
    for (size_t w = 0; w < ways; ++w) {
      if ((tags[idx * ways + w] & VALID) == 0) {
        q.push(w); 
        tags[idx * ways + w] = (addr >> idx_shift) | VALID;
        return 0; 
      }
    }
  }

  size_t victim_way = q.front(); 
  q.pop();
  q.push(victim_way);
...
{
  uint64_t tag_key = addr >> idx_shift;
  uint64_t old_tag = 0;

  if (tags.size() == ways) {
    uint64_t victim_key = fifo_queue.front(); 
    fifo_queue.pop();
    old_tag = tags[victim_key];
    tags.erase(victim_key);
  }

  tags[tag_key] = tag_key | VALID;
  fifo_queue.push(tag_key);
